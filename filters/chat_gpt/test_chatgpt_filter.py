...
def test_reads_history_from_repository():
    ...
    requestMessage: RequestMessage = RequestMessage('What is the meaning of life?', MOCK_USER_ID)
...
def test_builds_context_from_history():
    ...
    repository.get_last_n.return_value = [
        ...
        History(MOCK_ID, '1234-abcdef', 'What is the meaning of the universe?', '43'),
        History(MOCK_ID, '1234-abcdef', 'What is the meaning of everything?', '44'),
    ]
    actual = build_context_from_history(MOCK_USER_ID, repository)

    assert repository.get_last_n.called == True
    assert actual[0] == {'role': 'user', 'content': 'What is the meaning of life?'}
    assert actual[1] == {'role': 'assistant', 'content': '42'}
    assert actual[2] == {'role': 'user', 'content': 'What is the meaning of the universe?'}
    assert actual[3] == {'role': 'assistant', 'content': '43'}
    assert actual[4] == {'role': 'user', 'content': 'What is the meaning of everything?'}
